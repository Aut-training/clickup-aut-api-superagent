...
describe('Goals Api endpoint', function () {
  ...
  it('Verifies that there is 1 goal created in the WorkSpace', async function() {
    const goal1 = await clickUpApi.goals.createGoal(helper.team.id, helper.generateID(), helper.generateID());
    const goal2 = await clickUpApi.goals.createGoal(helper.team.id, helper.generateID(), helper.generateID());
  ...
  it('Verifies that there is a created goal in the WorkSpace', async function() {
    const goal = await clickUpApi.goals.getGoal(this.createdGoal.id);
    expect(goal.name).to.eq(this.createdGoal.name);
  });

  it('Verifies that a goal created has been updated', async function() {
