...
class Comments {
   ...
   * @example
   * // returns {"checklist": { "id": 15826580, "hist_id": 1764883347554751118:,
   *  "date": 1580991384622 }}
   * comments.createTaskComment("TaskId", "New Comment");
   */
  async createTaskComment(taskId, comment) {
    const taskComment = await this.request.post(
        `${this.baseUrl}/task/${taskId}${pathUrl}`,
        {comment_text: comment, assignee: null, notify_all: true},
  ...
  async createListComment(listId, comment) {
    const listComment = await this.request.post(
  ...
  async createViewComment(viewId, comment) {
    const viewComment = await this.request.post(
        `${this.baseUrl}/view/${viewId}${pathUrl}`,
   ...
   * @example
   * // returns [{id: 123, comment: [], comment_text...},
   *  {id: 456, comment: [], comment_text...}]
   * comments.getTaskComments("taskId");
   */
  async getTaskComments(taskId) {
    const taskComments = await this.request.get(
        `${this.baseUrl}/task/${taskId}${pathUrl}`,
        {archived: false},
        {'Authorization': this.apiKey},
    );
  ...
  async getListComments(listId) {
    const listComments = await this.request.get(
        `${this.baseUrl}/list/${listId}${pathUrl}`,
        {archived: false},
        {'Authorization': this.apiKey},
    );
  ...
  async getViewComments(viewId) {
    const viewComments = await this.request.get(
        `${this.baseUrl}/view/${viewId}${pathUrl}`,
        {archived: false},
        {'Authorization': this.apiKey},
    );
   ...
   * @return {Promise<Object>} Empty object if the update was successful.
   * @example
   * // returns {}
   * comments.updateComment("CommentId", "New Comment");
   */
  async updateComment(commentId, comment) {
    const updatedComment = await this.request.put(
        `${this.baseUrl}${pathUrl}/${commentId}`,
        {comment_text: comment, assignee: null, resolved: true},
        {'Authorization': this.apiKey},
    );
  ...
  async deleteComment(commentId) {
    const comment = await this.request.delete(
        `${this.baseUrl}${pathUrl}/${commentId}`,
        {archived: false},
        {'Authorization': this.apiKey},
    );
